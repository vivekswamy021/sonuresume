    ...
    if source_type == 'file':
        ...
        for edu in parsed_data.get('education', []):
            if isinstance(edu, dict):
                edu_str = f"Degree: {edu.get('degree', 'N/A')}, Institution: {edu.get('institution', 'N/A')}, University: {edu.get('university', 'N/A')}, Dates: {edu.get('dates', 'N/A')}"
...
# Helper function to convert unstructured LLM-parsed education list (strings) 
# into the new structured list of dicts format, falling back to string if needed.
def initialize_education_data(initial_data):
    """Initializes the session state structured education list."""
    
    # If the session state already has the structured list, use it
    if st.session_state.get('structured_education') is not None:
         return st.session_state.structured_education
    ...
    # Simple heuristic: Look for a comma or hyphen to split components
    for item in edu_list:
        if not isinstance(item, str): 
            item = str(item)
            
        # Example format: "B.S. Computer Science, Mock University, 2020"
        parts = [p.strip() for p in re.split(r'[,\-]', item)]
        
        degree = parts[0] if len(parts) > 0 else "N/A"
        institution = parts[1] if len(parts) > 1 else "N/A"
        dates = parts[-1] if len(parts) > 0 and re.search(r'\d{4}', parts[-1]) else "N/A"
        university = "N/A" # Cannot reliably parse a separate university field
...
def generate_cv_form():
    ...
    # --- CRITICAL: Initialize or load structured dynamic data ---
    ...
    # Display existing education entries with removal button (MUST be outside st.form for st.button to work)
    if st.session_state.structured_education:
        for i, edu in enumerate(st.session_state.structured_education):
            ...
            with col_display:
                st.markdown(f"**{i+1}. {edu['degree']}** from **{edu['institution']}** ({edu['dates']})")
                if edu['university'] and edu['university'] != 'N/A':
                    st.caption(f"University: {edu['university']}")
            with col_remove:
    ...
    with st.form("add_education_mini_form", clear_on_submit=True):
        col_new_edu1, col_new_edu2 = st.columns(2)
        
        with col_new_edu1:
            new_degree = st.text_input("Degree/Course Name (e.g., B.Tech, M.S. CS)", key="new_degree")
            new_institution = st.text_input("College/Institute Name", key="new_institution")
        with col_new_edu2:
            new_university = st.text_input("University Name (optional)", key="new_university")
            new_edu_dates = st.text_input("Dates From-To (e.g., 2018-2022)", key="new_edu_dates")
        ...
        if add_edu_submitted:
            if new_degree and new_institution and new_edu_dates:
                new_entry = {
                    "degree": new_degree.strip(),
                    "institution": new_institution.strip(),
                    "university": new_university.strip() or "N/A",
                    "dates": new_edu_dates.strip(),
                }
                st.session_state.structured_education.append(new_entry)
                st.success(f"Added: {new_degree} from {new_institution}")
                # Rerun is triggered by the form submission itself
            else:
                st.error("Please fill in Degree/Course, College/Institute, and Dates.")
    ...
    # --- MINI-FORM FOR ADDING NEW EXPERIENCE (Separate Form) ---
