...
# --- NEW FUNCTION: GITHUB/STREAMLIT PORTFOLIO TAB CONTENT ---
def portfolio_tab_content():
    st.header("💻 GitHub/Streamlit Portfolio")
    st.markdown("Manage and showcase your projects hosted on **GitHub** and **Streamlit**.")
    
    if 'github_repos' not in st.session_state:
        st.session_state.github_repos = []
        
    if 'streamlit_apps' not in st.session_state:
        st.session_state.streamlit_apps = []

    # --- 1. GitHub Repository Management ---
    st.subheader("1. GitHub Repositories 📂")
    
    with st.expander("Add New GitHub Repository Link"):
        with st.form("add_github_repo"):
            repo_url = st.text_input("GitHub Repository URL (e.g., https://github.com/user/repo)", key="new_repo_url")
            repo_desc = st.text_area("Short Description of the Project", key="new_repo_desc", height=80)
            repo_submit = st.form_submit_button("Add Repository")
            
            if repo_submit:
                if repo_url and repo_desc:
                    if repo_url.startswith("http") and "github.com" in repo_url:
                        st.session_state.github_repos.append({
                            "url": repo_url.strip(),
                            "description": repo_desc.strip(),
                            "name": repo_url.split('/')[-1]
                        })
                        st.success(f"Repository **{repo_url.split('/')[-1]}** added.")
                    else:
                        st.error("Please enter a valid GitHub URL.")
                else:
                    st.error("Please fill in both URL and Description.")
                    
    if st.session_state.github_repos:
        st.markdown("#### Your Added Repositories:")
        for i, repo in enumerate(st.session_state.github_repos):
            col_display, col_remove = st.columns([5, 1])
            with col_display:
                st.markdown(f"**[{repo['name']}]({repo['url']})**")
                st.caption(repo['description'])
            with col_remove:
                if st.button("🗑️ Remove", key=f"remove_repo_{i}"):
                    st.session_state.github_repos.pop(i)
                    st.success(f"Repository {repo['name']} removed.")
                    st.rerun()
    else:
        st.info("No GitHub repositories added yet.")

    st.markdown("---")
    
    # --- 2. Streamlit App Management ---
    st.subheader("2. Streamlit Applications 🚀")
    
    with st.expander("Add New Streamlit App Link"):
        with st.form("add_streamlit_app"):
            app_url = st.text_input("Streamlit App URL (e.g., https://share.streamlit.io/user/repo/app.py)", key="new_app_url")
            app_desc = st.text_area("Short Description of the App", key="new_app_desc", height=80)
            app_submit = st.form_submit_button("Add Application")
            
            if app_submit:
                if app_url and app_desc:
                    if app_url.startswith("http"):
                        st.session_state.streamlit_apps.append({
                            "url": app_url.strip(),
                            "description": app_desc.strip(),
                            "name": app_url.split('/')[-1]
                        })
                        st.success(f"App **{app_url.split('/')[-1]}** added.")
                    else:
                        st.error("Please enter a valid URL.")
                else:
                    st.error("Please fill in both URL and Description.")

    if st.session_state.streamlit_apps:
        st.markdown("#### Your Added Streamlit Apps:")
        for i, app in enumerate(st.session_state.streamlit_apps):
            col_display, col_remove = st.columns([5, 1])
            with col_display:
            ...
            with col_remove:
                if st.button("🗑️ Remove", key=f"remove_app_{i}"):
                    st.session_state.streamlit_apps.pop(i)
                    st.success(f"App {app['name']} removed.")
                    st.rerun()
    else:
        st.info("No Streamlit applications added yet.")
# --- END NEW FUNCTION ---
...
def candidate_dashboard():
    ...
    # Main Content Tabs (REARRANGED TABS HERE, NEW TAB ADDED)
    tab_cv_mgmt, tab_parsing, tab_portfolio, tab_jd_mgmt, tab_batch_match, tab_filter_jd, tab_chatbot, tab_interview_prep = st.tabs([
        "✍️ CV Management", 
        "📄 Resume Parsing", 
        "💻 GitHub/Streamlit Portfolio", # NEW TAB (Tab 3)
        "📚 JD Management", 
        "🎯 Batch JD Match",
        "🔍 Filter JD",
    ...
    # --- TAB 2: GitHub/Streamlit Portfolio (NEW TAB) ---
    with tab_portfolio:
    ...
    with tab_chatbot:
        ...
        # --- 6B. JD CHATBOT CONTENT (NEW SUBTAB) ---
        with sub_tab_jd:
...
def main():
    ...
    # NEW: GitHub/Streamlit Portfolio State
    if 'github_repos' not in st.session_state:
        st.session_state.github_repos = []
